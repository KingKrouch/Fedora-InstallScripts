...
var Config = class {
  constructor() {
    ...
    this.btreeInsertionPoint = kwin.readConfig("BTreeInsertionPoint", 0 /* Left */);
    ...
    this.resizeAmount = kwin.readConfig("ResizeAmount", 10) / 1e3;
    ...
    stackloop:
      while (stack.length > 0) {
        for (const node of stack) {
          if (node.children == null) {
            if (node.client != null) {
              if (config.btreeInsertionPoint == 2 /* Active */ && targetClient && node.client != targetClient) {
                continue;
              }
              node.split();
        ...
        stack = stackNext;
        if (config.btreeInsertionPoint == 1 /* Right */ && i % 2 == 0) {
          stack.reverse();
        }
        stackNext = [];
        i += 1;
...
var TilingEngine3 = class {
  ...
  addClient(client) {
    if (this.columns[1].length == 0) {
      ...
      this.columns[2].push(new Container2(client));
    } else {
      this.columns[0].push(new Container2(client));
    }
    return true;
  }
  putClientInTile(client, tile, direction) {
